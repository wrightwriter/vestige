     ...
     "for(float i=10.;i++<its+10.+min(T,0);)"
       "{"
         ...
         "for(float k=0.;k<kits+min(T,0);k++)"
           "{"
             "float g=offsc,ots=1.+smoothstep(0.,4.,tid)*2.*exp(-mod(T,1.))*smoothstep(0.,.004,mod(t,1.)),ot=pow(h(i+k*1.5+tid).x,2.+.5*smoothstep(5e2,2e3,g));"
             "g*=1.+floor(ot*ots);"
             "ots=exp(-t*(7.-6.*smoothstep(2e3,8e2,g))*.7)*smoothstep(replen*2.,0.,t)*smoothstep(0.,.002,t)*smoothstep(7e3,8e2,g);"
             "vec2 wave=vec2(sin(tau*g*TT))*rot(i);"
